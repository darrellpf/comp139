...
public class Tree<E extends Comparable<E>> implements Iterable<E> {
	...
	private class TreeIterator implements Iterator<E> {
		...
		private IteratorNode tail;
		
		public TreeIterator() {
			head = new IteratorNode(null); // add dummy
			tail = head;
			
			makeIteratorList(root);
		...
		private void makeIteratorList(TreeNode root) {
			if(root == null) {
				return;
			}
			
			makeIteratorList(root.leftSide);
			
			tail.next = new IteratorNode(root.item);
			tail = tail.next;
			
			makeIteratorList(root.rightSide);
